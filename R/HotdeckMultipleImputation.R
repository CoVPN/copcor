...
hotdeckMI <-
  function(X,
    ...
    if (length(Z2[epsilonz == 1 & is.na(Z2)]) > 0) {
      print("Problem 1")
    }
    if (length(Z2[epsilonv == 1 &
                  Delta == 1 & is.na(V)]) > 0) {
      print("Problem 2")
    }
    ...
    distsi <- function(x, ymat) {
      ...
      if (p == 0) {
        print("Problem in distsi with p=0")
      }
      
      xmat <- t(as.matrix(x[kpcols]))
      ymat <- ymat[, kpcols]
      if (m == 1) {
        ymat <- matrix(ymat, ncol = p)
      }
      if (p == 1) {
        ymat <- matrix(ymat, ncol = p)
      }
    ...
    nmissmark <-
      sum(Delta * (1 - epsilonv)) # Number of cases with a missing mark
    ...
    for (m in 1:M) {
      ...
      for (i in c(1:length(Delta))[Delta == 1 & epsilonv == 0]) {
        ...
        if (epsilonz[i] == 1 & epsilona[i] == 1) {
          ...
          mattminusi <-
            cbind(Xbt[availableneighborcases], Z1scalarbt[availableneighborcases, ], Z2bt[availableneighborcases], Avscalarbt[availableneighborcases, ])
          if (length(availableneighborcases[availableneighborcases]) == 1) {
            mattminusi <-
              matrix(c(Xbt[availableneighborcases], Z1scalarbt[availableneighborcases, ], Z2bt[availableneighborcases], Avscalarbt[availableneighborcases, ]),
                     ncol = length(vecti))
          }
          ...
          if (length(vecti) != ncol(mattminusi)) {
            print("Problem 7")
          }
        ...
        if (epsilonz[i] == 1 & epsilona[i] == 0) {
          ...
          mattminusi <-
            cbind(Xbt[availableneighborcases], Z1scalarbt[availableneighborcases, ], Z2bt[availableneighborcases])
          if (length(availableneighborcases[availableneighborcases]) == 1) {
            mattminusi <-
              matrix(c(Xbt[availableneighborcases], Z1scalarbt[availableneighborcases, ], Z2bt[availableneighborcases]),
                     ncol = length(vecti))
          }
          indsminusi <- c(1:NN)[availableneighborcases]
          distsavailableneighborcases <- distsi(vecti, mattminusi)
        ...
        if (epsilonz[i] == 0 & epsilona[i] == 1) {
          ...
          mattminusi <-
            cbind(Xbt[availableneighborcases], Z1scalarbt[availableneighborcases, ], Avscalarbt[availableneighborcases, ])
          if (length(availableneighborcases[availableneighborcases]) == 1) {
            mattminusi <-
              matrix(c(Xbt[availableneighborcases], Z1scalarbt[availableneighborcases, ], Avscalarbt[availableneighborcases, ]),
                     ncol = length(vecti))
          }
          indsminusi <- c(1:NN)[availableneighborcases]
          distsavailableneighborcases <- distsi(vecti, mattminusi)
        ...
        if (epsilonz[i] == 0 & epsilona[i] == 0) {
          ...
          mattminusi <-
            cbind(Xbt[availableneighborcases], Z1scalarbt[availableneighborcases, ])
          if (length(availableneighborcases[availableneighborcases]) == 1) {
            mattminusi <-
              matrix(c(Xbt[availableneighborcases], Z1scalarbt[availableneighborcases, ]),
                     ncol = length(vecti))
          }
          indsminusi <- c(1:NN)[availableneighborcases]
          distsavailableneighborcases <- distsi(vecti, mattminusi)
        ...
        Zscoredistsnearest[i, ] <-
          distsavailableneighborcases[order(distsavailableneighborcases)][1:L]
